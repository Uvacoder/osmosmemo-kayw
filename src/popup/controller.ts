...
export class Controller {
  ...
  async init() {
    ...
    try {
      ...
      this.model.update({ tagOptions: tagOptions.concat(newTagFileOptions.tags), fileOptions: fileOptions.concat(newTagFileOptions.files), libraryUrl, connectionStatus: "valid" });
  ...
  async onSave() {
    ...
    try {
      ...
      if (newTagFileOptions.tags.length || newTagFileOptions.files.length) {
        ...
        commitFiles.push({ path: manifest, content: resultContent  })
      }
